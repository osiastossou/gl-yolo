...
class AdaptA2C2f(nn.Module):
    """A2C2f avec area adaptatif appris. Le modèle apprend automatiquement : - area petit (4-8) sur P3 : petits objects
    denses - area grand (1-2) sur P5 : objects larges, contexte global.
    """
    ...
    def __init__(
        ...
        self.branches = nn.ModuleList(
            [nn.Sequential(*(ABlock(c_, c_ // 32, mlp_ratio, a) for _ in range(2))) for a in area_candidates]
        )
    ...
    def forward(self, x):
        ...
        outs = torch.stack([branch(feat) for branch in self.branches], dim=1)  # (B, n_areas, C', H, W)
