...
class LearnableLPPool2d(nn.Module):
    """
    Version apprenable du LPPool2d.
    Le paramètre 'p' est optimisé par rétropropagation pendant l'entraînement.
    """

    def __init__(self, kernel_size, stride=None, initial_p=2.0, ceil_mode=False):
        super(LearnableLPPool2d, self).__init__()
    ...
    def forward(self, input):
        ...
        # On utilise F.softplus ou clamp pour garantir que p ne devienne pas < 1
        p_clamped = torch.clamp(self.p, min=1.0)

        # On utilise la fonction fonctionnelle de PyTorch
        return F.lp_pool2d(
            input,
            float(p_clamped),
            self.kernel_size,
