...
def _patch_stats(x: torch.Tensor, k: int, s: int, p: int, d: int) -> torch.Tensor:
    ...
    Parameters
    ----------
    x : Tensor (B, C, H, W) k, s, p, d : kernel size, stride, padding, dilation (same as the conv)
    ...
    with torch.no_grad():
        ...
        # Normalize patches to [0,1] per spatial location for binning
...
class _KernelGenerator(nn.Module):
    ...
    To keep parameter count manageable, we factorise the kernel generation: the MLP outputs a per-channel scale vector
    (c_out values) and a shared spatial filter (c_in * k * k values). The full kernel is their outer product:

        W_{i,j}[c_out, c_in, k, k] = alpha[c_out] * phi[c_in, k, k]

    This reduces the MLP output from c_out * c_in * k² to c_out + c_in * k², cutting parameters by ~60× for typical
    sizes.

    Parameters
    ----------
    d_in : stat vector dimension (5) c_in : input channels of the convolution c_out : output channels k : spatial kernel
    size h_dim : hidden layer width (default 64)
    """
    ...
    def __init__(self, d_in: int, c_in: int, c_out: int, k: int, h_dim: int = 64):
        ...
        # Init near zero so SAC starts close to standard conv behavior
    ...
    def forward(self, stats: torch.Tensor) -> torch.Tensor:
        """Parameters ---------- stats : (B, 5, L) stop-gradient statistics.
...
class SACConv1(nn.Module):
    ...
    Drop-in replacement for ultralytics ``Conv``. Identical constructor signature.

    Parameters
    ----------
    c1 : int input channels c2 : int output channels k : int kernel size (default 1) s : int stride (default 1) p : int
    | None padding (default auto) g : int groups (default 1) — passed to fallback standard conv d : int dilation
    (default 1) act: bool | nn.Module activation flag (mirrors ultralytics Conv) h_dim : int MLP hidden width (default
    64, reduce to 16 to save FLOPs)
    """
    ...
    def __init__(
        ...
        self.base_conv = nn.Conv2d(c1, c2, k, s, self.padding, groups=g, dilation=d, bias=False)
        ...
        if self.use_sac:
            self.generator = _KernelGenerator(d_in=5, c_in=c1, c_out=c2, k=k, h_dim=h_dim)

    # ── helpers ──────────────────────────────────────────────────────────────

    def _apply_adaptive_kernels(self, x: torch.Tensor, kernels: torch.Tensor, H_out: int, W_out: int) -> torch.Tensor:
        ...
        # patches: (B, c1*k*k, L)
        patches = F.unfold(x, kernel_size=self.k, dilation=self.d, padding=self.padding, stride=self.s)
        # (B*L, c_in*k*k, 1)
