...
class GL_CAB1(nn.Module):
    """Global and Local Combined Attention Block. Bao Liu et al., IEEE DDCLS 2023 — DOI:
    10.1109/DDCLS58216.2023.10165957.
    ...
    Fix : la branche LDFE réduit H×W → 1×1 via GlobalAvgPool. BatchNorm2d crash quand B=1 car variance indéfinie sur 1
    valeur. Solution : GroupNorm(1, c) = LayerNorm sur les canaux, fonctionne quelle que soit la taille spatiale.
    """
...
class C2f_GLCAB1(nn.Module):
    """C2f avec GL_CAB — drop-in pour C3k2 / A2C2f dans YOLOv12. Signature identique à C3k2 pour le YAML.
    """
...
class GL_CAB(nn.Module):
    ...
    Signature : GL_CAB(c1, c2, *args, **kwargs) *args et **kwargs absorbent les arguments supplémentaires que
    parse_model peut envoyer selon la version d'Ultralytics.

    YAML : [-1, 1, GL_CAB, [1024]] parse_model envoie : GL_CAB(c1=ch[f], c2=1024)
    """
...
class C2f_GLCAB(nn.Module):
    ...
    parse_model insère n automatiquement dans args[2] quand C2f_GLCAB est dans repeat_modules.

    YAML : [-1, 2, C2f_GLCAB, [256, False, 0.25]] parse_model envoie : C2f_GLCAB(c1, c2, n=2, c3k=False, e=0.25)
    """

    def __init__(
        self, c1: int, c2: int, n: int = 1, c3k: bool = False, e: float = 0.5, shortcut: bool = True, **kwargs
    ):
        super().__init__()
        c_ = max(int(c2 * e), 32)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv((1 + n) * c_, c2, 1)
        self.m = nn.ModuleList([GL_CAB(c_, c_) for _ in range(n)])
