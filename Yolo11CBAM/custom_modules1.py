...
class WaveletAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = out.unfold(2, self.patch_size, self.patch_size).unfold(
            3, self.patch_size, self.patch_size
        )  # [B, 4C, nH, nW, pH, pW]
        patches = patches.contiguous().view(
            B, out.shape[1], -1, self.patch_size * self.patch_size
        )  # [B, 4C, num_patches, pH*pW]
        patches = (
            patches.permute(0, 2, 1, 3).contiguous().view(B, -1, self.patch_size * self.patch_size * out.shape[1])
...
class GL_CAB(nn.Module):
    ...
    def __init__(self, c1, c2=None):
        ...
        self.local_features_path = nn.Sequential(conv_block(c1, c1), nn.Hardswish())
...
class GL_CAB_Light(nn.Module):
    def __init__(self, c1, c2=None, reduction=4):
        ...
        self._global = nn.Sequential(nn.Conv2d(rc, rc, 1, bias=False), nn.BatchNorm2d(rc), nn.SiLU())
        self.local = nn.Sequential(nn.Conv2d(rc, rc, 1, bias=False), nn.BatchNorm2d(rc), nn.SiLU())
        self.detail = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), nn.Conv2d(rc, rc, 1, bias=False), nn.BatchNorm2d(rc), nn.SiLU()
        )
