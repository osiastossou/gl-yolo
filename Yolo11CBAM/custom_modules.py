...
class ChannelAttention(nn.Module):
    """Module d'attention sur les canaux (Channel Attention). Il apprend à donner plus de poids aux canaux de
    caractéristiques les plus pertinents.
    """
...
class SpatialAttention(nn.Module):
    """Module d'attention spatiale (Spatial Attention). Il apprend à se concentrer sur les régions spatiales les plus
    importantes.
    """
...
class CBAM(nn.Module):
    """Module CBAM complete que nous insérerons dans YOLOv8. Il applique séquentiellement l'attention canal puis
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads=4, num_layers=1):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=c1, nhead=num_heads, dim_feedforward=c1 * 4, dropout=0.1, activation="relu", batch_first=True
        )
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)

    def forward(self, x):
        # Le reste de la function forward est inchangé
        b, c, h, w = x.shape
...
class PatchEmbedding(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)  # "Découpage intelligent"
...
class GL_CAB1(nn.Module):
    def __init__(self, channels, kernel_size=3, fusion="add"):
        """channels: number of input channels (C) kernel_size: kernel for local conv (default 3) fusion: 'add' or 'cat'.
        """
        super().__init__()
...
class GL_CAB_ABlock(nn.Module):
    ...
    def __init__(self, c1, c2=None, n=2):
        ...
        self.m = nn.Sequential(*(ABlock(dim=self.c1, num_heads=self.c1 // 64, mlp_ratio=1.2, area=1) for _ in range(n)))
...
# Dilated Depthwise Pyramid (Learnable SPPF)


class DilatedDepthwisePyramid(nn.Module):
    """Learnable replacement of SPPF using Dilated Depthwise Convolutions.

    Structure:
        - 1x1 Conv (channel reduction)
        - 3 parallel depthwise conv with dilation = 1, 2, 3
        - Concatenation
        - 1x1 Conv (fusion)
    """

    def __init__(self, in_channels, out_channels, kernel_size=5, reduction=2):
        super().__init__()
        hidden_channels = in_channels // reduction

        # Channel reduction (like YOLO SPPF)
        self.cv1 = nn.Conv2d(in_channels, hidden_channels, 1, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(hidden_channels)
        self.act1 = nn.SiLU()

        # Dilated depthwise convolutions (multi-scale receptive field)
        ...
        # Final fusion
        self.cv2 = nn.Conv2d(hidden_channels * (len(self.dw_convs) + 1), out_channels, 1, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.act2 = nn.SiLU()

    def forward(self, x):
        x = self.act1(self.bn1(self.cv1(x)))
        dw_outs = [self.act_dw(self.bn_dw(dw_conv(x))) for dw_conv in self.dw_convs]
        out = torch.cat([x, *dw_outs], dim=1)
