...
def parse_args():
    ...
    parser.add_argument("--save-json", action="store_true",
                        help="Sauvegarder les résultats au format COCO JSON")
    parser.add_argument("--plots",  action="store_true",
                        help="Générer les courbes P/R et matrices de confusion")
...
def load_model(model_path: str) -> YOLO:
    """
    Charge un modèle YOLOv11.
    - Si le fichier .pt existe localement → charge depuis le disque
    - Sinon → télécharge automatiquement depuis Ultralytics Hub
    """
...
def run_validation(model: YOLO, args) -> dict:
    ...
    """
    print(f"[▶] Lancement de la validation...")
...
def print_metrics(results) -> None:
    ...
    print(f"\n{'═'*50}")
    print(f"  RÉSULTATS DE VALIDATION — YOLOv11")
    print(f"{'═'*50}")
